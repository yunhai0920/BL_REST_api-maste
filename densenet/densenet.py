...
def conv_block(inputs, growth_rate, dropout_rate=None, weight_decay=1e-4):
...
def transition_block(inputs, nb_filter, dropout_rate=None, pooltype=1, weight_decay=1e-4):
    x = BatchNormalization(axis=-1, epsilon=1.1e-5)(inputs)
    x = Activation('relu')(x)
...
def dense_cnn(inputs, nclass):
    ...
    x = Conv2D(_nb_filter, (5, 5), strides=(2, 2), kernel_initializer='he_normal', padding='same',
               use_bias=False, kernel_regularizer=l2(_weight_decay))(inputs)
   
    # 64 + 8 * 8 = 128
    x, _nb_filter = dense_block(x, 8, _nb_filter, 8, 0., _weight_decay)
    # 128
